...
async def send_nutrition(message: types.Message):
    try:
        ...
        prompt = (
             f"Составь строгий план питания на 3 дня для мужчины:\n"
    f"- Вес: {user_data['weight']} кг\n"
    f"- Рост: {user_data['height']} см\n"
    f"- Возраст: {user_data['age']} лет\n"
    f"- Цель: {user_data['goal']}\n\n"
    "Требования к ответу:\n"
    "1. Только конкретные блюда без вариативности\n"
    "2. Четкие грамовки всех ингредиентов\n"
    "3. Три приема пищи в день (завтрак, обед, ужин)\n"
    "4. Подсчет КБЖУ для каждого дня\n\n"
    "Формат ответа (обязательно):\n"
    "=== День 1 ===\n"
    "Завтрак:\n"
    "- 100г овсянки на молоке (50г овсянки + 150мл молока 2.5%)\n"
    "- 2 яйца всмятку (120г)\n"
    "- 30г миндаля\n"
    "Обед:\n"
    "- 150г куриной грудки гриль\n"
    "- 100г гречки (сухой вес)\n"
    "- 200г овощного салата (огурцы, помидоры, зелень)\n"
    "Ужин:\n"
    "- 200г запеченной горбуши\n"
    "- 150г бурого риса\n"
    "- 100г тушеной брокколи\n"
    "КБЖУ дня:\n"
    "- Калории: 2500 ккал\n"
    "- Белки: 180г\n"
    "- Жиры: 90г\n"
    ...
    "Ответ должен содержать ровно 3 дня, без сокращений и комментариев."
        )

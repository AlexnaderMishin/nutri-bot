...
async def send_nutrition(message: types.Message):
    try:
        ...
        prompt = (
            f"Составь детальный план питания для мужчины:\n"
            f"- Вес: {user_data['weight']} кг\n"
            f"- Рост: {user_data['height']} см\n"
            f"- Возраст: {user_data['age']} лет\n"
            f"- Цель: {user_data['goal']}\n\n"
            "Формат ответа (обязательно разделяй на блоки):\n"
            "1. Калорийность: XXXX ккал\n"
            "2. БЖУ: Белки XXг, Жиры XXг, Углеводы XXг\n"
            "3. Рекомендации по питанию (3-5 пунктов)\n"
            "4. Пример меню на день (завтрак, обед, ужин, перекусы)"
        )
        ...
        for section, emoji in sections.items():
            if section in plan:
                content = plan.split(section)[1].split("\n\n")[0].strip()
                formatted_sections.append(f"{emoji} *{section}:*\n{content}")
        
        # Улучшенное форматирование
        formatted_plan = (
            f"🍏 *ПЕРСОНАЛЬНЫЙ ПЛАН ПИТАНИЯ* �\n\n"
            f"*📊 Ваши параметры:*\n"
            f"• 🏋️‍♂️ Вес: {user_data['weight']} кг\n"
            f"• 📏 Рост: {user_data['height']} см\n"
            f"• 🎂 Возраст: {user_data['age']} лет\n"
            f"• 🎯 Цель: {user_data['goal']}\n\n"
        ...
        await message.answer("⚠️ Ошибка генерации плана. Попробуйте позже.")

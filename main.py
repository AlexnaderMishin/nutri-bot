...
async def send_nutrition(message: types.Message):
    try:
        ...
        prompt = (
            f"Составь детальный план питания для мужчины со следующими параметрами:\n"
            f"- Вес: {user_data['weight']} кг\n"
            f"- Рост: {user_data['height']} см\n"
            f"- Возраст: {user_data['age']} лет\n"
            f"- Цель: {user_data['goal']}\n\n"
            "Ответ должен содержать следующие разделы, каждый должен начинаться с заголовка в квадратных скобках:\n"
            "[Калорийность]\n"
            "Расчет суточной нормы калорий с формулой (указать формулу и коэффициенты)\n"
            "Итоговая рекомендуемая калорийность: XXXX ккал\n\n"
            "[БЖУ]\n"
            "Рекомендуемое распределение белков, жиров и углеводов в граммах и процентах\n"
            "Белки: XXг (XX%)\n"
            "Жиры: XXг (XX%)\n"
            "Углеводы: XXг (XX%)\n\n"
            "[Рекомендации]\n"
            ...
            "...\n\n"
            "[Меню]\n"
            "Завтрак: описание (XX ккал)\n"
            "Обед: описание (XX ккал)\n"
            "Ужин: описание (XX ккал)\n"
            "Перекус 1: описание (XX ккал)\n"
            "Перекус 2: описание (XX ккал)\n\n"
            "Все цифры должны быть рассчитаны на основе предоставленных данных!"
        )
        ...
        if not response or 'choices' not in response:
            raise Exception("Invalid GigaChat response: no choices in response")

        plan = response['choices'][0]['message']['content']
        logger.debug(f"Received plan from GigaChat: {plan}")
